...
class FetchGeonames:
    ...
    username (str, optional): Geonames username, defaults to "horoscope.bot".
    ...
    def __init__(
            self, city_name: str,
            country_code: str,
            username: str = "horoscope.bot",
            logger: Union[logging.Logger, None] = None
    ):

        redis_conn = redis.from_url(config.redis_dsn)
        redis_backend = RedisCache(connection=redis_conn)
    ...
    def __get_contry_data(self, city_name: str, country_code: str) -> dict[str, str]:
        ...
        try:
            ...
            city_data_whitout_tz['cityName'] = response_json['geonames'][0]['toponymName']

            city_names = response_json['geonames'][0]['alternateNames']
            for city in city_names:
                if city['lang'] == 'ru':
                    city_data_whitout_tz['cityName'] = city['name']
...
if __name__ == "__main__":
    geonames = FetchGeonames("Moscow", "IT")
