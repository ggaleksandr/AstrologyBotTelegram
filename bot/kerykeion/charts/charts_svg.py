...
from bot.kerykeion.aspects import NatalAspects, CompositeAspects
from bot.kerykeion.kerykeion import KrInstance
from bot.kerykeion.typesKerykeion import KerykeionException, ChartType, KerykeionSubject, LunarPhaseObject
...
class MakeSvgInstance:
    ...
    def __transitRing(self, r):
        out = '<circle cx="%s" cy="%s" r="%s" style="fill: none; stroke: %s; stroke-width: 36px; stroke-opacity: .9;"/>' % (
            r, r, r-18, self.colors_settings['paper_1'])
        out += '<circle cx="%s" cy="%s" r="%s" style="fill: none; stroke: %s; stroke-width: 1px; stroke-opacity: .9;"/>' % (
    ...
    def __makeHouses(self, r):
        ...
        for i in range(xr):
            ...
            if self.chart_type == "Transit" or self.chart_type == "Composite":
                ...
                if self.chart_type == "Transit":
                    path = path + '<text style="fill: #00f; fill-opacity: 0; font-family:Sans,Arial; font-size: 14px"><tspan x="' + \
                    ...
                    path = path + '<text style="fill: #00f; fill-opacity: .4; font-family:Sans,Arial; font-size: 14px"><tspan x="' + \
                        str(xtext-3)+'" y="'+str(ytext+3) + \
                        '">'+str(i+1)+'</tspan></text>'
                    path = path + '<line x1="'+str(t_x1)+'" y1="'+str(t_y1)+'" x2="'+str(t_x2)+'" y2="'+str(
            ...
            path = path + '<text style="fill: #433931; fill-opacity: .6; font-family:Montserrat,Arial; font-size: 24px"><tspan x="' + \
                str(xtext-3)+'" y="'+str(ytext+3) + \
                '">'+str(i+1)+'</tspan></text>'
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            for e in range(len(t_keys)):
                ...
                output += '<text transform="rotate(%s)" text-anchor="%s' % (
                    rotate, textanchor)
                output += '" style="fill: ' + \
                    self.planets_settings[i]['color']+'; font-family:Sans,Arial; font-size: 10px;">' + \
        ...
        * Yod: Two qunicunxes together joined by a sextile.
        ...
        if len(yot) >= 1:
            ...
            for k, v in yot.items():
                out += '<text y="%s" style="fill:%s; font-family:Sans,Arial; font-size: 12px;">%s</text>' % (
    ...
    def __makeAspectTransitGrid(self, r):
        ...
        out += '<text y="-15" x="0" style="fill:%s; font-family:Sans,Arial; font-size: 14px;">%s</text>' % (
        ...
        for i in range(len(self.aspects_list)):
            ...
            out += '<text y="8" x="45" style="fill:%s; font-family:Sans,Arial; font-size: 10px;">%s</text>' % (
                self.colors_settings['paper_0'],
    ...
    def __makeElements(self, r):
        ...
        out += '<text y="0" style="fill:#ff6600; font-family:Sans,Arial; font-size: 10px;">' + \
            self.language_settings['fire']+'  '+str(pf)+'%</text>'
        out += '<text y="12" style="fill:#6a2d04; font-family:Sans,Arial; font-size: 10px;">' + \
            self.language_settings['earth']+' '+str(pe)+'%</text>'
        out += '<text y="24" style="fill:#6f76d1; font-family:Sans,Arial; font-size: 10px;">' + \
            self.language_settings['air']+'   '+str(pa)+'%</text>'
        out += '<text y="36" style="fill:#630e73; font-family:Sans,Arial; font-size: 10px;">' + \
    ...
    def __makePlanetGrid(self):
        ...
        out += \
            f'<text text-anchor="end" style="fill:{self.colors_settings["paper_0"]}; font-family:Sans,Arial; font-size: 14px;">{self.language_settings["planets_and_house"]} {self.name}:</text>'
        out += '</g>'
        ...
        for i in range(len(self.planets_settings)):
            ...
            if self.planets_settings[i]['visible'] == 1:
                ...
                # planet text
                out += f'<text text-anchor="end" style="fill:{self.colors_settings["paper_0"]}; font-family:Sans,Arial; font-size: 10px;">{self.language_settings["planets"][self.planets_settings[i]["label"]]}</text>'
                # planet symbol
                out += \
                    '<g transform="translate(5,-8)"><use transform="scale(0.4)" xlink:href="#' + \
                    self.planets_settings[i]['name']+'" /></g>'
                # planet degree
                out += '<text text-anchor="start" x="19" style="fill:%s; font-family:Sans,Arial; font-size: 10px;">%s</text>' % (
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            if self.chart_type == "Transit":
                ...
                out += \
                    f'<text text-anchor="end" style="fill:{self.colors_settings["paper_0"]}; font-family:Sans,Arial; font-size: 14px;">{self.t_name}:</text>'
            else:
                out += '<g transform="translate(380, -15)">'
                out += \
                    f'<text text-anchor="end" style="fill:{self.colors_settings["paper_0"]}; font-family:Sans,Arial; font-size: 14px;">{self.language_settings["planets_and_house"]} {self.t_user.name}:</text>'
            out += '</g>'
    ...
    def __makeHousesGrid(self):
        ...
        for i in range(12):
            ...
            out += '<text text-anchor="end" x="40" style="fill:%s; font-family:Sans,Arial; font-size: 10px;">%s %s:</text>' % (
            ...
            out += '<text x="53" style="fill:%s; font-family:Sans,Arial; font-size: 10px;"> %s</text>' % (
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            td['c1style'] = 'fill: none; stroke: %s; stroke-width: 2px; ' % (
                self.colors_settings['zodiac_radix_ring_2'])
            td['c2'] = 'cx="' + str(r) + '" cy="' + \
                str(r) + '" r="' + str(r-self.c2) + '"'
            td['c2style'] = 'fill: %s; fill-opacity:.9; stroke: %s; stroke-opacity:.4; stroke-width: 2px' % (
                self.colors_settings['paper_1'], self.colors_settings['zodiac_radix_ring_1'])
            td['c3'] = 'cx="' + str(r) + '" cy="' + \
                str(r) + '" r="' + str(r-self.c3) + '"'
            td['c3style'] = 'fill: %s; fill-opacity:.8; stroke: %s; stroke-width: 2px' % (
                self.colors_settings['paper_1'], self.colors_settings['zodiac_radix_ring_0'])
            td['c4'] = 'cx="' + str(r) + '" cy="' + \
                str(r) + '" r="' + str(r-self.c4) + '"'
            td['c4style'] = 'fill: %s; fill-opacity:.8; stroke: %s; stroke-width: 0px' % (
                self.colors_settings['paper_2'], self.colors_settings['zodiac_radix_ring_0'])
        ...
        if (deg < 90.0):
            ...
            lffg, lfbg = self.colors_settings["paper_1"], self.colors_settings["paper_1"]
        ...
        td['lunar_phase_outline'] = self.colors_settings["paper_1"]
        ...
        if self.chart_type == "Composite":
            #print("self.t_user.name", self.t_user.name)
    ...
    def returnSVG(self):
        """Return the SVG file as bytearray"""

        if not (self.template):
            self.template = self.makeTemplate()

        byteString = bytearray(self.template, 'utf-8')
